				     ...
				     size_t context_length)
{
	...
	if (PSA_ALG_IS_JPAKE(operation->alg) && IS_ENABLED(PSA_NEED_CRACEN_ECJPAKE_SECP_R1_256)) {
		...
		status = cracen_spake2p_set_context(&operation->cracen_spake2p_ctx, context, context_length);
	}
					...
					size_t key_buffer_size, size_t *key_buffer_length)
{
	...
	if (PSA_ALG_IS_JPAKE(operation->alg) && IS_ENABLED(PSA_NEED_CRACEN_ECJPAKE_SECP_R1_256)) {
		status = cracen_jpake_get_shared_key(&operation->cracen_jpake_ctx, key_buffer,
						     key_buffer_size, key_buffer_length);
	} else if (PSA_ALG_IS_SRP_6(operation->alg) && IS_ENABLED(PSA_NEED_CRACEN_SRP_6)) {
		status = cracen_srp_get_shared_key(&operation->cracen_srp_ctx, key_buffer,
						   key_buffer_size, key_buffer_length);
	} else if (PSA_ALG_IS_SPAKE2P(operation->alg) && IS_ENABLED(PSA_NEED_CRACEN_SPAKE2P)) {
		status = cracen_spake2p_get_shared_key(&operation->cracen_spake2p_ctx, key_buffer,
						       key_buffer_size, key_buffer_length);
	}
